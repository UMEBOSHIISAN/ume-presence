...
class NativeProcessAudioListener {
  constructor({
    ...
    this.failedCaptureKey = null;
    this.failedCaptureError = null;
    this.pollTimer = null;
  ...
  async poll() {
    ...
    try {
      ...
      if (this.failedCaptureKey !== null) return;
  ...
  startCapture(processIds, key) {
    ...
    child.once("error", (error) => {
      ...
      this.failedCaptureError = error.message;
      this.reportStatus({
        available: false,
        capturing: false,
        monitoring: true,
    ...
    child.once("exit", (code, signal) => {
      ...
      this.capture = null;
      this.captureKey = null;
      if (!this.stopped && this.failedCaptureKey === null) {
        this.failedCaptureKey = key;
        this.failedCaptureError = code
          ? `Native listener exited with code ${code}${signal ? ` (${signal})` : ""}.`
          : "Native listener exited unexpectedly.";
      }
      this.gate.reset();
      this.reportStatus({
        available: true,
        ...
        source: null,
        ...(this.failedCaptureKey === key && this.failedCaptureError
          ? { error: this.failedCaptureError }
          : {}),
      });
    });
  }
  ...
  handleHelperMessage(child, message) {
    ...
    if (message.type === "error") {
      this.failedCaptureKey = this.captureKey;
      this.failedCaptureError = String(message.message || "Native listener failed.");
      this.reportStatus({
        available: false,
        capturing: false,
        monitoring: true,
